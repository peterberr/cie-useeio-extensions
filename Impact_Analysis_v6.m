...
switch year
    case 2007
        load(['USEEIO_v7_2007_use.mat'])
      
    case 2013
        load(['USEEIO_v7_2013_use.mat'])
end
...
labels={'Pri. Expenditure', 'Gov. Expenditure', 'Pri. Investment','Gov. Investment'}; % legend labels
...
switch year
    case 2007
        title(['A. Aggregate Carbon Footprint, expenditure & capital investment, ' num2str(year)])
    case 2013
        title(['B. Aggregate Carbon Footprint, expenditure & capital investment, ' num2str(year)]) 
...
order=[4 3 2 1];
labels={'Pri. Expenditure', 'Gov. Expenditure', 'Pri. Capital Consumption','Gov. Capital Consumption'};
...
switch year
    case 2007
        title(['C. Aggregate Carbon Footprint, expenditure & capital consumption ' num2str(year)])
    case 2013 
        title(['D. Aggregate Carbon Footprint, expenditure & capital consumption ' num2str(year)])
...
switch year
    case 2007
        title(['A. Max Carbon Footprint, DIG, expenditure & capital consumption ' num2str(year)])
    case 2013
        title(['B. Max Carbon Footprint, DIG, expenditure & capital consumption ' num2str(year)])
        u=legend(q(order),labels{order});
        set(u,'Fontsize',8.3)
...
order=[4 3 2 1];
labels={'Pri. Expenditure', 'Gov. Expenditure', 'Pri. Investment','Gov. Investment'};
...
switch year
    case 2007
        title(['E. Aggregate Material Footprint, expenditure & capital investment, ' num2str(year)])
    case 2013
        title(['F. Aggregate Material Footprint, expenditure & capital investment, ' num2str(year)])
        u=legend(q(order),labels{order});
        set(u,'Fontsize',8.3)
end
colormap(cm);
...
switch year
    case 2007
        title(['G. Aggregate Material Footprint, expenditure and capital consumption ' num2str(year)])
    case 2013
        title(['H. Aggregate Material Footprint, expenditure and capital consumption ' num2str(year)])
...
switch year
    case 2007
        title(['C. Max Material Footprint, DIG, expenditure & capital consumption ' num2str(year)])
    case 2013
        title(['D. Max Material Footprint, DIG, expenditure & capital consumption ' num2str(year)])
        u=legend(q(order),labels{order});
        set(u,'Fontsize',8.3)
...
switch year
    case 2007
        title(['I. Aggregate Energy Footprint, expenditure & capital investment, ' num2str(year)])
    case 2013
        title(['J. Aggregate Energy Footprint, expenditure & capital investment, ' num2str(year)])
        u=legend(q(order),labels{order});
        set(u,'Fontsize',8.3)
end
colormap(cm);
...
switch year
    case 2007
        title(['K. Aggregate Energy Footprint, expenditure & capital consumption ' num2str(year)])
    case 2013
        title(['L. Aggregate Energy Footprint, expenditure & capital consumption ' num2str(year)])
        u=legend(q5(order),labels{order});
        set(u,'Fontsize',8.3)
end
colormap(cm);
...
switch year
    case 2007
        title(['E. Max Energy Footprint, DIG, expenditure & capital consumption ' num2str(year)])
    case 2013
        title(['F. Max Energy Footprint, DIG, expenditure & capital consumption ' num2str(year)])
        u=legend(q(order),labels{order});
        set(u,'Fontsize',8.3)
...
xticklabels(xlab)
xtickangle(45)
labels=[Z(main_pr,3) ;{'Direct*'};{'Other'}];
...
k_pr=[22 281 133 14 123 20 29 23 21 36 34 31]; % most important production sectors for K assets
...
GHG_int_capcon=GHG_int_Atot-GHG_int_con; % GHG intensity of capital consumption
...
int=table(row,GHG_int_pr,GHG_int_con,GHG_int_Atot,GHG_dir_pc,GHGi_inc,GHGi_cc,M_int_A,M_int_At,Mi_inc,E_int_A,E_int_At,Ei_inc,Gabs_inc,GHG_pc_inc,Mabs_inc,Eabs_inc,'RowNames',Z(:,3)); % table of intensities and changes
...
order= [5 4 3 2 1];
labels={'2007 total','Pri. Expenditure', 'Gov. Expenditure', 'Pri. Investment','Gov. Investment'};
...
labels={'2007 total','Pri. Expenditure', 'Gov. Expenditure', 'Pri. Capital Consumption','Gov. Capital Consumption'};
...
title(['B. Aggregate Carbon Footprint, expenditure & capital consumption ' num2str(year)])
u=legend([q(order(2:end)) r],labels{order});
set(u,'Fontsize',9)
colormap(cm);
...
title(['F. Aggregate Material Footprint, expenditure & capital consumption, ' num2str(year)])
u=legend([q(order(2:end)) r],labels{order});
set(u,'Fontsize',9)
colormap(cm);
...
title(['C. Aggregate Energy Footprint, expenditure & capital investment, ' num2str(year)])
u=legend([q(order(2:end)) r],labels{order});
set(u,'Fontsize',9)
colormap(cm);
...
title(['D. Aggregate Energy Footprint, expenditure & capital consumption, ' num2str(year)])
u=legend([q(order(2:end)) r],labels{order});
set(u,'Fontsize',9)
colormap(cm);
...
% CF
y1=zeros(2,15,4);
y1(1,:,:)=sum_con_sort_Mt(1:15,1:4); % 2013 impacts
yr=R07.sum_con(order13,:); % 2007 impacts, use 2013 highest CF consumption sector order
y1(2,:,:)=1e-9*yr(1:15,1:4); % 2007 impacts, convert kg to Mt
...
title('A. Max Carbon Footprint, DIG, expenditure & capital consumption 2007/2013')
ylabel('Mt CO2eq')
ylim([0 1700])
set(gca,'FontSize',11.5);
colormap(cm);
...
% EF
y1=zeros(2,15,4);
y1(1,:,:)=EF_con_sort(1:15,1:4); % 2013 impacts
yr=R07.EF_con(ordere13,:); % 2007 impacts, use 2013 highest EF consumption sector order
y1(2,:,:)=yr(1:15,1:4); % 2007 impacts
...
xtickangle(45)
u=legend(q(1,order),labels{order});
title('B. Max Energy Footprint, DIG, expenditure & capital consumption 2007/2013')
ylabel('EJ')
ylim([0 25])
set(gca,'FontSize',11.5);
colormap(cm);
...
%MF
y1=zeros(2,15,4);
y1(1,:,:)=MF_con_sort_Mt(1:15,1:4); % 2013 impacts
yr=R07.MF_con(orderm13,:); % 2007 impacts, use 2013 highest MF consumption sector order
y1(2,:,:)=1e-9*yr(1:15,1:4); % 2007 impacts, convert kg to Mt
y2=permute(y1,[2,1,3]); % rearrange 3D array
...
xtickangle(45)
u=legend(q(1,order),labels{order});
title('C. Max Material Footprint, DIG, expenditure & capital consumption 2007/2013')
ylabel('Mt material')
ylim([0 750])
ax=gca;
set(gca,'FontSize',11.5);
