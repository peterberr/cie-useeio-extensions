...
% edit_USEEIOv1.1_Matrices.xlsx, See SI1.docx for description of modifications to Ingwersen 2017(https://doi.org/10.23719/1369615)
% Satellite files See SI XXX, modified version of Ingwersen et al. 2017 (https://doi.org/10.23719/1365565)
% edit_USEEIOv1.1_Satellite_GHG_SI_2007.xlsx, 
% edit_USEEIOv1.1_Satellite_GHG_SI_2013.xlsx
% edit_USEEIOv1.1_Satellite_Energy_2007.xlsx
% edit_USEEIOv1.1_Satellite_Energy_2013.xlsx
% edit_USEEIOv1.1_Satellite_Minerals_2007.xlsx
% edit_USEEIOv1.1_Satellite_Minerals_2013.xlsx
%
% From QP SUT projections (Kondo)
% balance_us_sut)07r_basic.xlsx
...
switch year
    ...
    case 2013
        ...
        [SUT2013, txtSUT2013, rawSUT2013]=xlsread('balance_us_sut_07r_basic.xlsx','Detail_QP');
end
...
%           load('FD_changes.mat')
          ...
          [MV_changes, FD_struc]=FD_changes(Zsort);
          ...
          comm_fd_dom(27:36,:)=FD_struc; % alter investment in structures
          comm_fd_dom(150:163,:)=comm_fd_dom(150:163,:)-MV_changes; % alter Motor vehicles
...
[C, txtC, ~] = xlsread('edit_USEEIOv1.1_Matrices.xlsx' , 'C'); % load edited characterisation matrix - includes modifications to material impact categories
S = xlsread('edit_USEEIOv1.1_Matrices.xlsx' , 'B'); % load edited stressor matrix - includes fuel wood and industrial roundwood biomass material extractions
...
switch year 
    case 2007
        S_2013USD=S;
        %GHG
        [GHG, ~, GHGraw] = xlsread('edit_USEEIOv1.1_Satellite_GHG_SI_2007.xlsx','US_all');
        ...
        [G13] = xlsread('edit_USEEIOv1.1_Satellite_GHG_SI_2007.xlsx','US_all_2013USD');
        ...
        %Energy
        [E, ~, Eraw]= xlsread('edit_USEEIOv1.1_Satellite_Energy_2007.xlsx','matrix_2007');
        Efull=zeros(389,9);
        E(isnan(E))=0;
        ...
        % create intensities per 2013USD
        [E13, ~, Er13]= xlsread('edit_USEEIOv1.1_Satellite_Energy_2007.xlsx','matrix_2013');
        E13full=zeros(389,9);
        E13(isnan(E13))=0;
        ...
        % Materials
        [M, ~, Mraw] = xlsread('edit_USEEIOv1.1_Satellite_Minerals_2007.xlsx','newmatrix_2007');
        Mfull=zeros(389,45);
        M(isnan(M))=0;
        ...
        % create intensities per 2013USD
        [M13, ~, Mr13] = xlsread('edit_USEEIOv1.1_Satellite_Minerals_2007.xlsx','newmatrix_2013');
    ...
    case 2013
        [GHG, ~, GHGraw] = xlsread('edit_USEEIOv1.1_Satellite_GHG_SI_2013.xlsx','US_all');
        lab=GHGraw(3:391,2); % industry labels from satellite tables
        lab=string(lab);
        GHGmat=GHG(1:389,34:48);
        Shh=GHG(390:392,34:48); % household fuel direct emissions
        ...
        Scsort=sortrows([order Sghg],1);
        Sc=Scsort(:,2:end);
        S(11:25,:)=Sc; % replace portion of S matrix with GHG emissions with 13 data    
      
        %Energy
        [E, ~, Eraw]= xlsread('edit_USEEIOv1.1_Satellite_Energy_2013.xlsx','matrix');
        Efull=zeros(389,9);
        E(isnan(E))=0;
        estr=[string(Eraw(3:end,1)) string(E(:,2:end))];
        % create matrix of stressors per sector
        ...
        S(2:10,1:389)=Ec;   
        %Materials
        [M, ~, Mraw] = xlsread('edit_USEEIOv1.1_Satellite_Minerals_2013.xlsx','newmatrix');
        Mfull=zeros(389,45);
        M(isnan(M))=0;
        mstr=[string(Mraw(4:end,2)) string(M(3:end,2:end))];
        %create matrix of stressors per sector
...
pipe=M1(180,3)-M1(180,7); % difference for pipe transportation
...
% food=M7(172,3);
% general=M7(173,3);
% other=M7(174,3)-M7(174,7);
% sum_Retail=motor+food+general+other;
% Tex(275,:)=(motor/sum_Retail)*texs(:,4)'; % distribute inputs of motor dealers to final demand
% Tex(276,:)=(food/sum_Retail)*texs(:,4)'; % food and beverage stores
% Tex(277,:)=(general/sum_Retail)*texs(:,4)'; % general merch stores
% Tex(288,:)=(other/sum_Retail)*texs(:,4)'; % Other retail
% allocate wholesale margins
